...
| 3001–3012 | 数据源 | `3001` 数据源不存在 / `3002` 名称重复 / `3003` 连接失败 / `3004` 驱动未安装 / `3010` SQL 被白名单拦截 / `3011` 执行失败 / `3012` 行数超上限 |
...
> 安全:默认白名单仅放行只读类语句(SELECT/SHOW/DESCRIBE/EXPLAIN),被拦截返回 `3010`;DML(INSERT/UPDATE/DELETE)需后端白名单开启。结果行数超过上限返回 `3012`(需加 LIMIT)。
