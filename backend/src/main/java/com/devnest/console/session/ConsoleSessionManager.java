...
public class ConsoleSessionManager {
    ...
    public void openSession(String wsSessionId, Long consoleId, Consumer<String> onOutput) {
        ...
        try {
            ...
            SshTunnelInstance tunnel = null;
            String connectHost;
            int connectPort;

            if (config.getBastionId() != null) {
                // 隧道模式:经跳板隧道连内网主机
                tunnel = tunnelManager.getInstance(config.getBastionId());
                if (tunnel == null || tunnel.getState() != TunnelState.RUNNING) {
                    throw new IllegalStateException("跳板隧道未启动,请先启动隧道再开终端");
                }
                allocatedPort = portAllocator.allocateAny();
                tunnel.addDynamicForwarding(allocatedPort, config.getRemoteHost(), config.getRemotePort());
            ...
            ConsoleSession cs = new ConsoleSession(wsSessionId, channel, session,
                    allocatedPort, tunnel, portAllocator, onOutput);
            cs.start();
